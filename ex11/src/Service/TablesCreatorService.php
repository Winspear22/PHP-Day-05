...
{
    public function createAllTables(Connection $connection): array
    {
        $messages = [];
        try
        {
            $messages[] = $this->createPersonsTable($connection, 'ex11_persons');
            $messages[] = $this->createBankAccountsTable($connection, 'ex11_bank_accounts');
            $messages[] = $this->createAddressesTable($connection, 'ex11_addresses');
            return $messages;
        }
        catch (Throwable $e)
        {
